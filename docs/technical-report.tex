...
\author{Sailesh \\ \small \texttt{dsailesh2001@gmail.com}}
...
standardized vocabularies. We report three contributions beyond the
implementation itself. First, we show that a single blended
concept-mapping-coverage statistic can misrepresent pipeline correctness,
and we present a decomposed metric that separates source-data coverage
...
records carry a mappable code. Second, we describe a synthetic-fixture
...
redistributed, so CI cannot use real vocabulary content. Third, we document
...
johnson2023mimiciv}. We treat this as an engineering and methodology report
...
We suggest this decomposition -- coverage among rows carrying a
...
cross-table analytic questions, we ran a query joining
...
We state these plainly rather than minimize them. \texttt{MEASUREMENT}
...
We presented a verified, open FHIR-to-OMOP CDM v5.4 ETL pipeline covering
...
assumed. Beyond the implementation, we argue for decomposing
concept-mapping coverage metrics to separate source-data ceilings from
mapping-logic defects, and we describe a synthetic-fixture CI methodology
...
\url{https://github.com/Saileshwar1625/fhir-omop-etl} (tag \texttt{v1.0}).
